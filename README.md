...
mysql -uroot -p123456 teacher_agent < backend/src/main/resources/db/migration/V2__course_config.sql
mysql -uroot -p123456 teacher_agent < backend/src/main/resources/db/migration/V3__knowledge_base.sql
mysql -uroot -p123456 teacher_agent < backend/src/main/resources/db/migration/V4__kb_folder.sql
mysql -uroot -p123456 teacher_agent < backend/src/main/resources/db/migration/V5__qa_session.sql
```

> 升级提示：如果数据库 `llm_config.api_key` 字段中存在历史 `enc:` 前缀的加密残留数据（旧版本曾有 AES 加密路径，现已废弃），系统启动后管理员需进入「模型配置」页清空后重新填写明文 Key。也可手动执行清理：
> ```sql
> UPDATE llm_config SET api_key='', last_test_status='untested', last_test_message=NULL WHERE api_key LIKE 'enc:%';
> ```
...
```bash
cd backend
# 修改 application.yml 中的数据库密码和端口（或通过环境变量 TEACHER_AGENT_DB_PASSWORD / JWT_SECRET 覆盖）
...
1. 登录管理员账号
2. 进入「模型配置」页面，**填入至少一家厂商的明文 API Key**（init.sql 默认已清空所有 Key），点「保存」→「测试」→「激活」
3. 进入「课程配置」页面，配置您教授的课程信息
4. 开始使用教案生成或题库生成功能

> 注意：「模型配置」页面只有管理员可见。普通教师登录后无法看到该入口；如需变更模型，请联系管理员。
>
> 当前测试支持的厂商默认接口：
> - **DeepSeek**: `https://api.deepseek.com/v1`，模型 `deepseek-chat`
> - **智谱 GLM**: `https://open.bigmodel.cn/api/paas/v4`，模型 `glm-4-flash` / `glm-4-plus`
> - **Kimi**: `https://api.moonshot.cn/v1`，模型 `moonshot-v1-32k`
> - **通义千问**: `https://dashscope.aliyuncs.com/compatible-mode/v1`，模型 `qwen-plus` / `qwen-max`
> - **MiniMax**: `https://api.minimax.chat/v1`，模型 `MiniMax-M2.5-highspeed`（已统一使用 OpenAI 兼容协议 `/chat/completions`）
